...
module.exports = function (app) {
  ...
  app.command('/task-edit', async ({ ack, body, client, logger }) => {
    ...
    try {
      ...
      else console.error('[task-edit] Slack API error (modal open):', apiErr);
  ...
  app.view(
    ...
    async ({ ack, body, view, client, logger }) => {
      ...
      const botToken = await getTokenForTeam(body.team.id);
      if (!botToken) {
        console.error('No bot token found for workspace:', body.team.id);
        return;
      }
      const realClient = new WebClient(botToken);
      try {
        ...
        const result = await db
          ...
          .updateOne(
            { _id: taskId },
            { $set: { description: newDesc, due_date: newDue } },
          );
