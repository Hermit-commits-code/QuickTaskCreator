    ...
    db.run(
      ...
      function (err) {
        if (err) {
          ...
          client.chat.postMessage({
            channel: creatorId,
            text: `:white_check_mark: Task created: *${description}* (Assigned to: <@${assignedUser}>) (Due: ${dueDate})`,
          });
          // Notify assigned user
          let assignedMsg = "";
          if (assignedUser === creatorId) {
            assignedMsg = `:memo: You created a new personal task: *${description}* (Due: ${dueDate})`;
          } else {
            assignedMsg = `:bell: You’ve been assigned a new task by <@${creatorId}>: *${description}* (Due: ${dueDate})`;
          }
          client.chat.postMessage({
