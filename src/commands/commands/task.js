...
module.exports = function (app) {
  ...
  app.command('/task', async ({ ack, client, body, logger }) => {
    ...
    if (!botToken) {
      if (logger)
        logger.error('[task] No bot token found for workspace:', workspace_id);
      else
        console.error('[task] No bot token found for workspace:', workspace_id);
    ...
    try {
      ...
      if (logger) logger.error('[task] Slack API error (modal open):', apiErr);
